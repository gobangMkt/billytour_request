...
function checkDuplicate(tourUrl, product) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var rowProd = String(data[i][5]).trim();
...
/* ───────────────────────────────────────────
   신청 폼 저장
   컬럼: A=신청일시, B=결제완료일, C=이름, D=연락처,
         E=빌리투어URL, F=상품선택, G=추가요청사항,
         H=결제링크발송(발송대기/발송하기/발송완료), I=발송시간
─────────────────────────────────────────── */
function submitForm(formData) {
  ...
  if (sheet.getLastRow() === 0) {
    sheet.appendRow(['신청일시', '결제완료일', '이름', '연락처', '빌리투어 URL',
                     '상품 선택', '추가 요청사항', '결제링크 발송', '발송시간']);
  ...
  // H열 "발송대기" 명시 (체크박스 서식 잔재 방지)
  var newRow = sheet.getLastRow();
  sheet.getRange(newRow, 8).setValue('발송대기');

  // ── 작업 내역 시트에도 기록 (제작 관리용) ──
  // A=신청일시, B=이름, C=연락처, D=상품, E=빌리투어URL,
  // F=작업상태, G=결과물URL, H=완료일, I=완료알림발송, J=발송시간
  var workSheet = ss.getSheetByName('작업 내역');
  if (workSheet) {
    if (workSheet.getLastRow() === 0) {
      workSheet.appendRow(['신청일시', '이름', '연락처', '상품', '빌리투어 URL',
                           '작업 상태', '결과물 URL', '완료일', '완료알림 발송', '발송시간']);
      workSheet.getRange(1, 1, 1, 10).setFontWeight('bold').setBackground('#f0f0f0');
    }
    workSheet.appendRow([
      now, formData.name || '', formData.phone || '', productLabel,
      formData.tourUrl || '', '대기', '', '', '발송대기', ''
    ]);
    workSheet.getRange(workSheet.getLastRow(), 9).setValue('발송대기');
  }
...
function handlePaymentSend(e, sheet, row) {
  ...
  var phone   = String(rowData[3]).replace(/[^0-9]/g, '');
  var name    = String(rowData[2] || '').trim();
...
/* 작업 내역 I열: 완료 알림톡 발송 (결과물 URL + 완료일 입력 시에만) */
function handleCompleteSend(e, sheet, row) {
  if (String(e.range.getValue()).trim() !== '발송하기') return;

  // J열(발송시간)이 이미 기록돼 있으면 재발송 안 함
  ...
  var resultUrl = String(rowData[6] || '').trim();   // G 결과물 URL
  var doneDate  = rowData[7];                          // H 완료일
...
/* ───────────────────────────────────────────
   ...
   · 신청 내역 H열(8)  · 작업 내역 I열(9)
─────────────────────────────────────────── */
function setupDropdowns() {
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID);

  var apply = ss.getSheetByName('신청 내역');
  if (apply) applyStatusDropdown(apply, 8);

  var work = ss.getSheetByName('작업 내역');
  if (work) {
    applyStatusDropdown(work, 9);
    applyWorkStatusDropdown(work, 6); // F열 작업 상태
  }
...
function setupSheets() {
  ...
  if (apply.getLastRow() === 0) {
    apply.appendRow(['신청일시', '결제완료일', '이름', '연락처', '빌리투어 URL',
                     '상품 선택', '추가 요청사항', '결제링크 발송', '발송시간']);
    apply.getRange(1, 1, 1, 9).setFontWeight('bold').setBackground('#f0f0f0');
  }

  // 작업 내역 헤더
  var work = ss.getSheetByName('작업 내역');
  if (work.getLastRow() === 0) {
    work.appendRow(['신청일시', '이름', '연락처', '상품', '빌리투어 URL',
                    '작업 상태', '결과물 URL', '완료일', '완료알림 발송', '발송시간']);
    work.getRange(1, 1, 1, 10).setFontWeight('bold').setBackground('#f0f0f0');
  }
