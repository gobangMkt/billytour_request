var SPREADSHEET_ID    = '1dwLBuPaKjlQ5dle2-fAPPdPkVf3PPQrwdNP0D_KYhas';
...
function submitForm(formData) {
  ...
  // 중복 체크 — 릴스는 상품 단일이므로 라벨 고정값으로 비교
  ...
  var now = new Date();
  // 릴스 전용 — 상품 분기 없이 항상 '릴스단건' 고정 (product 코드 'R'로 들어와도 무시)
...
function handlePaymentSend(e, sheet, row) {
  ...
  // 릴스 전용 — 결제링크는 릴스 템플릿 버튼에 박힘 (상품 분기 없음)
  var templateId = TEMPLATE_PAYMENT_REELS;
...
function handleCompleteSend(e, sheet, row) {
  ...
  var phone      = String(rowData[2]).replace(/[^0-9]/g, ''); // C 연락처
  var name       = String(rowData[1] || '').trim();           // B 이름
  // 릴스 전용 — 완료 알림은 릴스/인스타 업로드 완료 템플릿 (상품 분기 없음)
  var templateId = TEMPLATE_COMPLETE_REELS;
...
// 결제요청 알림톡(릴스 템플릿) 발송 + J='발송완료'·K=발송시간. K열 있으면 재발송 가드.
