...
| D | 상품 (글로벌재구매/숏츠단건) | 자동 |
| E | 빌리투어 URL | 자동 |
| F | 작업 상태 (대기/촬영/편집/업로드/완료) | 드롭박스 |
| G | 결과물 URL (유튜브/숏츠 링크) | 수동 |
| H | 완료일 | 수동 |
| I | 완료알림 발송 — `발송대기`/`발송하기`/`발송완료` | 드롭박스 |
| J | 발송시간 | 자동 |

**I열 완료알림 로직 (절대 규칙):**
- `발송하기` 선택 → **G(결과물URL) + H(완료일) 둘 다 입력된 경우만** 완료 알림톡 발송 후 `발송완료`
- 미입력 시 → `발송대기`로 자동 복귀
